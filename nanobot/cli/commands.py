...
from nanobot import Nanobot, __logo__, __version__  # noqa: E402
...
# -- Direct capability invocation (bypasses LLM) — spec tool-invocation-interface#5, #6 --


call_app = typer.Typer(help="Invoke a capability deterministically (no LLM).")
capabilities_app = typer.Typer(help="List and describe business capabilities.")
...
def call_callback(
    ...
    """Direct capability invocation (bypasses LLM)."""
    if ctx.invoked_subcommand is None:
        # No subcommand: show help
        console.print(ctx.get_help())


@call_app.command(name="run")
def call_command(
    tool_name: str = typer.Argument(..., help="Name of the capability/tool to invoke."),
    params: str = typer.Option("{}", "--params", help="JSON params, e.g. '{\"k\":\"v\"}'"),
    ...
    """Invoke a capability directly via Nanobot.call() (sync, no LLM)."""
    import json as _json
    try:
    ...
    async def _run() -> "object":
        ...
        try:
            return await bot.acall(tool_name, parsed, timeout=timeout)
        finally:
            await bot.aclose()
    ...
    if output_format == "text":
        console.print(str(result.content if result.ok else result.error))
    else:
        # JSON: serialize ToolResult to dict
        out = {
            "ok": result.ok,
...
def capabilities_list(
    mode: str = typer.Option(None, "--mode", help="Filter: capability | both | tool | all"),
) -> None:
    """List business capabilities."""
    bot = Nanobot.from_config()
    items = bot.list_capabilities(mode=mode)
    if not items:
        console.print("[dim]No capabilities found.[/dim]")
        return
    for item in items:
        console.print(f"[bold cyan]{item.name}[/bold cyan]  {item.description}")


@capabilities_app.command("describe")
def capabilities_describe(
    tool_name: str = typer.Argument(..., help="Name of the capability to describe."),
    show_example: bool = typer.Option(False, "--example", help="Include example params if available."),
) -> None:
    """Describe a single capability: name / description / parameters / meta."""
    bot = Nanobot.from_config()
    info = next((i for i in bot.list_capabilities(mode="all") if i.name == tool_name), None)
    if info is None:
    ...
    payload = {
        ...
        "meta": {
            "invocation_mode": info.meta.invocation_mode,
            "is_long_running": info.meta.is_long_running,
            "is_idempotent": info.meta.is_idempotent,
            "requires_auth": sorted(info.meta.requires_auth),
            "timeout_s": info.meta.timeout_s,
            "side_effect_class": info.meta.side_effect_class,
            "owner": info.meta.owner,
            "version": info.meta.version,
            "status": info.meta.status,
            "tags": sorted(info.meta.tags),
        },
    }
    if show_example:
        # Tools may expose class-level `example_params` (P3 cap-marketplace convention)
        example = getattr(bot._loop.tools.get(tool_name), "example_params", None) if bot._loop else None
        if example is not None:
            payload["parameters"]["example"] = example
